...
> В задании написано:
> 
> 1. Во время подготовки к сдаче задания необходимо:
> 2. Обновить пакет profi_service_pkg_1 до последней версии в удаленном репозитории;
...
![modB pic3](img/modB_pic3.png)

Примерный вывод (если нечего было обновлять):
![modB pic4](img/modB_pic4.png)
...
> *если появилась ошибка ModuleNotFoundError: No module named 'tqdm'*</mark>
...
<mark>ВНИМАНИЕ!</mark> сейчас в инструкции к пакету в гите написано: `roslaunch profi_service_pkg_2 start_configure.launch`
...
> 
> - Робот TurtleBro;
> - Комплект расширения для сборки полезной нагрузки "Робот-инспектор": тепловизор AMG88хх 8x8 Thermal Camera Sensor, сигнальная лампа;
> - Дополнительное программное обеспечение: мета-пакет [turtlebro_extra](https://github.com/voltbro/turtlebro_extra/)


> Ссылка на bag файл для тренировки: https://disk.yandex.ru/d/pXZzE-OTv0Vg6g﻿
...
![modB pic12](img/modB_pic12.png)

Примерно так должен выглядеть ваш результат:
![modB pic13](img/modB_pic13.png)
...
2. ноутбук готов к работе. на нем нужно запустить ros ядро (на роботе мы это не делаем, потому что рос запускается сразу вместе с роботом)
```bash
roscore
```
окно терминала с сервером ROS **не закрываем** (окно №1)!

3. В терминале переходим в папку, где лежит ваш файл .bag (cd <адрес папки>)

Выполняем команду `rosbag play <название bag файла>`:
![modB pic14](img/modB_pic14.png)

Получаем вывод:
![modB pic15](img/modB_pic15.png)

4. Файл сразу начнет проигрываться. **Нажимаем пробел**, чтобы поставить на паузу. Окно не закрываем (окно №2)

5. Открываем **новое** окно терминала (окно №3) (можно просто вкладку), в нем запускаем `rviz`:
![modB pic16](img/modB_pic16.png)
...
6. Открываем еще одно окно терминала (окно №4) (В ОТДЕЛЬНОМ ОКНЕ - НЕ ВКЛАДКУ). В нем нам нужно будет просмотреть показания с тепловизора. Они публикуются в топик `/amg88xx_pixels`, поэтому включаем прослушивание этого топика:
```bash
rostopic echo /amg88xx_pixels
```
(ничего не будет происходить, потому что проигрывание файла пока на паузе)

7. Открываем еще одно окно терминала (четвертое) (В ОТДЕЛЬНОМ ОКНЕ - НЕ ВКЛАДКУ). В нем нам нужно будет просмотреть координаты робота. Они публикуются в топик `/odom_pose2d` (в другие тоже, но в этом удобнее), поэтому включаем прослушивание этого топика:
```bash
rostopic echo /odom
```
(ничего не будет происходить, потому что проигрывание файла пока на паузе)

8. Удобно располагаем 3 окна: rviz, терминал с данными тепловизора, терминал с координатами. Затем открываем вкладку, в которой на паузе стоит воспроизведение bag файла и нажатием на пробел снимаем с паузы:
![modB pic17](img/modB_pic17.png)
...
<mark>Альтернативный вариант задания Б3:</mark>
...
> !Очень важно следить за тем, что выводит ваша программа и что она должна выводить по заданию. Если формат вывода не соотвествует, но значения правильные, то задание зачтено не будет
...
```
Наконец, сохраняем текстовый файл с полученной таблицей (по заданию нужно именно сохранить в файл, а не вывести в терминал):
```python
# Сохраняем в файл
...
with open('paintings_table.txt', 'w', encoding='utf-8') as f:
    ...
    for line in table_lines:
        ...
        f.write(line + '\n')
```
Итоговый код (в конце добавила еще пару строчек после сохранения для доп. вывода в терминал, но это не обязательно):
```python
# Открываем файлы, кодировка в конце указывается, чтобы не было ошибок
log1 = open('aruco_detect_server-1.log', encoding='utf-8')
log2 = open('speech_service-3.log', encoding='utf-8')
